    ...
    <main className="publication-shell pb-8 pt-8 sm:pt-10">
      <section className="grid gap-7 border-b border-stone-300 pb-8 lg:grid-cols-[minmax(0,1.5fr),minmax(18rem,0.7fr)] lg:items-end">
        <div>
          ...
          <p className="mt-5 max-w-3xl text-lg leading-7 text-stone-700">{site.tagline}</p>
        </div>
        <div className="border-l border-stone-300 pl-5 text-sm leading-6 text-stone-600">
          ...
          <p className="mt-2">Monitoreo de fuentes externas, selección determinística y análisis propio publicados como capas distintas.</p>
      ...
      <OutletPulse curatedCount={curated.length} latestCount={signals.latest.length} topicCount={signals.sections.length} />
        ...
        {featured ? (
          <div className="mt-6 grid gap-6 lg:grid-cols-[minmax(0,1.65fr),minmax(17rem,0.65fr)]">
            <article className="editorial-callout p-6 sm:p-8">
              ...
              <h2 className="mt-4 max-w-4xl text-4xl font-semibold leading-[1.05] sm:text-5xl"><Link href={`/articles/${featured.slug}`} className="article-link">{featured.title}</Link></h2>
                ...
                <div className="mt-2 divide-y divide-stone-200">
                  {moreAnalysis.slice(0, 4).map((item: any) => (
                    <article key={item.article_id} className="py-4">
                      ...
                      <Link href={`/articles/${item.slug}`} className="article-link mt-2 block text-lg font-semibold leading-snug">{item.title}</Link>
                    </article>
                  ))}
                </div>
              ) : <p className="mt-4 text-sm leading-6 text-stone-600">Esta es la única pieza aprobada en el snapshot actual.</p>}
            </aside>
          </div>
        ) : (
          <div className="editorial-callout mt-6 max-w-4xl p-6 text-sm leading-6 text-stone-600">
            ...
            <p className="mt-2">No hay análisis editorial aprobado publicado en este corte. Las señales de fuentes externas se muestran por separado debajo y no se presentan como contenido propio.</p>
          </div>
        )}
      </section>
      ...
      <section className="border-t border-stone-300 py-10 sm:py-12">
        ...
        <div className="mt-3 flex flex-wrap items-end justify-between gap-5">
          <div>
            ...
            <p className="mt-2 max-w-3xl text-sm leading-6 text-stone-600">Shortlist determinística de señales externas según actualidad, prioridad temática y diversidad. Estar seleccionado no convierte una señal en autoría, aprobación ni análisis editorial de Media Monitor.</p>
          </div>
          <Link href="/methodology" className="text-xs font-semibold uppercase tracking-[0.08em] text-[#8d2b2b] underline underline-offset-4">Ver metodología</Link>
        </div>
          ...
          <div className="mt-7 grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            {curated.slice(0, 6).map((item: any) => (
              <article key={item.index_id} className="signal-card flex min-h-[18rem] flex-col p-5">
                ...
                <Link href={`/story/${item.index_id}`} className="story-link mt-3 block text-xl font-semibold leading-snug">{item.title}</Link>
                <div className="mt-auto flex flex-wrap gap-1.5 pt-5">
                  {item.reason_codes.slice(0, 3).map((code: string) => <span key={code} className="chip">{curationReasonLabel(code)}</span>)}
                </div>
              </article>
            ))}
          </div>
        ) : (
          <div className="mt-6 max-w-3xl border border-stone-300 bg-[#fffdf8] p-5 text-sm text-stone-600">Este despliegue todavía no contiene una selección editorial determinística. El cable cronológico sigue disponible debajo sin ser presentado como una shortlist curada.</div>
        )}
      </section>
        ...
        <div>
          <div className="flex items-end justify-between gap-4">
            <div>
              ...
              <h2 className="mt-3 text-3xl font-semibold">Cable cronológico de fuentes</h2>
            </div>
            <Link href="/latest" className="text-xs font-semibold uppercase tracking-[0.08em] underline underline-offset-4">Ver todas</Link>
          </div>
            ...
            {signals.latest.map((item: any) => (
              <article key={item.index_id} className="wire-row">
                ...
                <div>
                  ...
                  <Link href={`/story/${item.index_id}`} className="story-link mt-1 block text-lg font-semibold leading-snug sm:text-xl">{item.title}</Link>
                </div>
              </article>
            ))}
          </div>
        ...
        <aside className="lg:border-l lg:border-stone-300 lg:pl-7">
          ...
          <div className="mt-4 border-t border-stone-300">
            {signals.sections.map((section: any) => (
              <Link key={section.topic} href={`/topic/${encodeURIComponent(section.topic)}`} className="topic-link">
                <span>{section.topic}</span><span className="font-semibold text-stone-500">{section.article_count}</span>
              </Link>
            ))}
          </div>
          <div className="mt-8 bg-[#20201c] p-5 text-stone-100">
            ...
            <Link href={EDITORIAL_IDENTITY.routes.journalists} className="mt-4 inline-block text-xs font-bold uppercase tracking-[0.08em] underline underline-offset-4">Para periodistas →</Link>
          </div>
        </aside>
      </section>
    </main>
