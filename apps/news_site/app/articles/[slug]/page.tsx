  ...
  return (
    <div className="mt-8 space-y-5 text-[1.05rem] leading-8 text-neutral-800">
      {blocks.map((block, index) => {
        if (block.startsWith("### ")) {
          return <h3 key={index} className="pt-2 text-xl font-semibold">{block.slice(4)}</h3>;
        }
        if (block.startsWith("## ")) {
          return <h2 key={index} className="pt-3 text-2xl font-semibold">{block.slice(3)}</h2>;
        }
        if (block.startsWith("# ")) {
          return <h2 key={index} className="pt-3 text-2xl font-semibold">{block.slice(2)}</h2>;
        }
    ...
    <main className="mx-auto max-w-4xl px-6 py-10">
      ...
      <article className="mt-8">
        ...
        <p className="mt-3 text-sm text-neutral-500">
          {outlet.site.name} · {article.topic}
        </p>
