...
def run() -> int:
    ...
    for (label, start, end) in slices:
        ...
        for _, r in df_slice.iterrows():
            ...
            if not ok:
                total_bad += 1
                bio.append_jsonl(quarantine_path("V01", run_id), {"reason": reason, "row": r.to_dict(), "digest_id": digest_id, "window_type": label})
                continue
        ...
        for _, r in gdf.iterrows():
            ...
            if not dry_run:
                try:
                    ...
                    bio.append_jsonl(quarantine_path("V01", run_id), {"reason": f"enqueue_error:{type(e).__name__}", "error": str(e), "row": r.to_dict(), "digest_id": digest_id})
