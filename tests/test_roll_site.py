        ...
        json.dumps(
            {
                ...
                "digest_at": digest,
                "snapshot_id": "a" * 64,
                "metrics": {"item_count": 11, "section_count": 1},
            }
        )
    )
    return path
...
class Fake:
    ...
    def __call__(self, command, *, cwd, env=None):
        ...
        if command[:2] == ["vercel", "curl"]:
            self.health_calls += 1
            value = (
                self.health[self.health_calls - 1]
...
def latest(root):
    return json.loads(
        next((root / "storage/observability").glob("site_roll_latest_*.json")).read_text()
    )
...
def test_successful_preview_roll(tmp_path):
    ...
    fake = Fake(tmp_path)
    record, code = roll(
        "argentina-general", "20260721T18", "preview", tmp_path, fake, lambda _: None
    )
    assert code == 0 and record["status"] == "ok"
    assert ["vercel", "pull", "--yes", "--environment=preview"] in [
...
def test_production_command_construction_and_freshness_record(tmp_path):
    ...
    fake = Fake(tmp_path)
    record, code = roll(
        "argentina-general", "20260721T18", "production", tmp_path, fake, lambda _: None
    )
    assert code == 0
...
def test_production_stale_health_fails(tmp_path):
    snapshot(tmp_path)
    health = default_health()
    health["publication_health"] = healthy_publication(
        freshness_status="STALE", is_current=False, within_target=False, age_minutes=500
    )
    record, code = roll(
        "argentina-general",
        "20260721T18",
...
def test_snapshot_mismatch_fails_before_vercel(tmp_path):
    ...
    fake = Fake(tmp_path)
    record, code = roll(
        "argentina-general", "20260721T18", "preview", tmp_path, fake
    )
...
def test_build_failure(tmp_path):
    ...
    record, code = roll(
        ...
        "preview",
        tmp_path,
        Fake(tmp_path, fail=["vercel", "build"]),
    )
...
def test_missing_output(tmp_path):
    ...
    def runner(command, **kwargs):
        return Result(
            command,
            0,
            "https://x.vercel.app" if command[:2] == ["vercel", "deploy"] else "",
            "",
        )

    record, code = roll(
        "argentina-general", "20260721T18", "preview", tmp_path, runner
    )
    assert code and record["failed_stage"] == "build"
...
def test_url_extraction_and_missing_host(tmp_path):
    ...
    record, code = roll(
        ...
        "preview",
        tmp_path,
        Fake(tmp_path, deploy="log https://x.vercel.app end"),
    )
...
def test_health_retries_and_failure_record_is_safe(tmp_path):
    ...
    record, code = roll(
        ...
        "preview",
        tmp_path,
        Fake(tmp_path, deploy="token=secret"),
        lambda _: None,
    )
    assert code == 1
...
def test_no_ingestion_or_editorial_commands(tmp_path):
    ...
    assert all(
        not any(
            word in " ".join(call[0])
            for word in ("s01", "editorial", "ingestion", "export-pr3a")
        )
