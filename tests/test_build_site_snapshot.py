...
def test_all_topics_wildcard_accepts_concrete_sensing_topics(tmp_path):
 config(tmp_path); inputs(tmp_path,topic='Inflación y Precios'); run(tmp_path); snap=json.loads((tmp_path/'out.json').read_text()); assert snap['metrics']=={'item_count':5,'section_count':1}; assert snap['hero']['topic']=='Inflación y Precios'
